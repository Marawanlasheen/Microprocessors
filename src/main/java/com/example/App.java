...
private int userLoadLatency = 2;
    ...
    public void start(Stage stage) {
        ...
        userAddLatency = getUserInt("ADD Latency (cycles)", 5);
        userSubLatency = getUserInt("SUB Latency (cycles)", 2);
        userMulLatency = getUserInt("MUL Latency (cycles)", 6);
        userDivLatency = getUserInt("DIV Latency (cycles)", 8);
        userLoadLatency = getUserInt("LOAD Latency (cycles)", 3);
        userBranchLatency = getUserInt("BRANCH Latency (cycles)", 2);
    ...
    stage.setTitle("Tomasulo Simulator");        // Prompt user for cache parameters
        ...
        userMissPenalty = getUserInt("Cache Miss Penalty (cycles)", 6);
    ...
    private void initCoreWithDefaults() {
        ...
        memory.initDoubleWord(8, 2L);  // 8 bytes at address 100-107 (big-endian)
        memory.initDoubleWord(16, 4L);  // Memory[120] = 25 (4 bytes at 120-123)
        memory.initDoubleWord(24, 5L);      
    ...
    private void showRegisterInitDialog() {
        ...
        TextArea regArea = new TextArea("F2=2");
